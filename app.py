...
def superadmin_home():
    if session.get('role') == 'superadmin' and 'user' in session:
...
def admin_home():
    ...
    reports_today = Report.query.filter(Report.created_at >= datetime.combine(date.today(), datetime.min.time())).count()
...
def trainee_home():
    if session.get('role') == 'trainee' and 'user' in session:
...
def get_trainee_info(trainee_id):
    ...
    return {
        ...
        "created_at": trainee.created_at.strftime('%Y-%m-%d %H:%M') if trainee.created_at else "—",
        "reports": [
